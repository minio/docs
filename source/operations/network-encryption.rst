...
``cert-manager`` Certificate Management
   MinIO Operator 5.0.x and later support using `cert-manager <https://cert-manager.io/>`__ 1.7.x or later for configuring Tenant TLS.
   The Tenant CRD spec ``spec.externalCertsSecret`` supports specifying a secret of type ``cert-manager.io/v1``.

   Once configured, the MinIO Operator uses the ``cert-manager`` configuration to provision TLS certificates for the MinIO Tenant.
   ``cert-manager`` can also support automatic renewal of TLS certificates upon expiry.
