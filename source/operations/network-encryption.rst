...
.. cond:: k8s
   ...
   #. Create a new secret in the ``minio-operator`` namespace named ``operator-ca-tls``.
   #. Add a ``ca.crt`` key to this secret with the public certificate for your internal CA.

   Once the secret is created, Operator creates a copy in each tenant namespace and mounts its certificates in the tenant pods.
