...
#  s390x arch
	...
	MINIOS390X=$(cat /tmp/downloads-minio.json | jq '.Linux."MinIO Server".s390x.Binary.download')
	DEBS390X=$(cat /tmp/downloads-minio.json | jq '.Linux."MinIO Server".s390x.DEB.download')
	RPMS390X=$(cat /tmp/downloads-minio.json | jq '.Linux."MinIO Server".s390x.RPM.download')
